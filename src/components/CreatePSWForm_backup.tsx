...
export function CreatePSWForm({ poNumber, pstNumber, pswWebSeqId, pswData, onClose, onSubmit, user }: CreatePSWFormProps) {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (response && !response.error && response.data) {
          ...
          setHeaderData({
            ...
            poDate: data.poDate || '',
            etd: '', // This field might not be in the API response
            eta: data.eta || '',
            wrDate: '', // This field might not be in the API response  
            invoiceNo: data.invoiceNo || '',
            awbNo: data.awbNo || '',
            pstBook: '', // This field might not be in the API response
            pstNo: '', // This field might not be in the API response
            status: data.billStatus || '',
            pswBook: '', // This field might not be in the API response
            pswNo: '' // This field might not be in the API response
          });
        }
      } catch (error) {
    ...
    if (pswWebSeqIdParam && modeParam === 'update') {
      ...
      const webSeqId = parseInt(pswWebSeqIdParam);
      if (!isNaN(webSeqId)) {
      ...
      <div className="p-6 space-y-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="space-y-8">
            ...
            {/* Existing expense content goes here */}
              <div className="flex items-center gap-2">
                <span className="text-gray-600">PO:</span>
